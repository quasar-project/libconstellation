  ...
  namespace netif
  {
    ...
    inline IF_ADDRS_UNICAST getNextAddress(int family, IF_ADDRS_UNICAST addrs)
    {
      ...
      while(next != nullptr)
      {
        ...
        const auto next_sockaddr = getSockAddr(next);
        if(next_sockaddr == nullptr)
        {
          next = next->ifa_next;
          continue;
        }

        const int next_family = next_sockaddr->sa_family;
     ...
     */
    inline std::vector<std::string> getInterfaceAddresses(const int family)
    {
      ...
      for(auto a = addrs; a != nullptr; a = getNextAddress(family, a))
      {
        if(auto ipAddr = addressToString(getSockAddr(a), getSockAddrLen(a)); !ipAddr.empty())
        {
     ...
     */
    inline auto getInterfaceAddressesAll() -> std::vector<std::string>
    {
